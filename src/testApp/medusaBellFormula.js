import {cos, float, mix, sin, vec3, vec2, time, smoothstep} from "three/tsl";
...
export const getBellPosition = (phase, zenith, azimuth, bottomFactor = 0) => {
    ...
    const zenithNoise = mx_perlin_noise_float(vec3(sinAzimuth.mul(3), cosAzimuth.mul(3), time));
    const modifiedZenith = zenith.mul(zenithNoise.mul(0.05).add(0.9)).toVar();
