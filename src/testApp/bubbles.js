...
export class Bubbles {
    ...
    constructor(renderer){
        ...
        this.material.positionNode = Fn(() => {
            ...
            const n0 = mx_perlin_noise_vec3(noisePos.add(noiseOffset.xzz)).mul(0.5).add(0.5);
            const n1 = mx_perlin_noise_vec3(noisePos.add(noiseOffset.yzz)).mul(0.5).add(0.5);
            const n2 = mx_perlin_noise_vec3(noisePos.add(noiseOffset.zxz)).mul(0.5).add(0.5);
            const n3 = mx_perlin_noise_vec3(noisePos.add(noiseOffset.zyz)).mul(0.5).add(0.5);
            const noiseTangent = n1.sub(n0);
            const noiseBitangent = n3.sub(n2);
            const noiseNormal = cross(noiseBitangent, noiseTangent).normalize();
